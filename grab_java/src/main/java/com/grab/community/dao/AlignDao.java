...
public class AlignDao {
	...
	public List<Board> viewASC(int boardType, Board b, Connection conn){
		...
		int count = 0;
		try {
			String sql = "SELECT * FROM `board` b JOIN `member` m ON b.member_no = m.member_no WHERE `board_type` = ? ORDER BY `hits` ASC LIMIT "+b.getLimitPage()+", "+b.getBoardList();
	...
	public List<Board> viewDESC(int boardType, Board b, Connection conn){
		...
		int count = 0;
		try {
			String sql = "SELECT * FROM `board` b JOIN `member` m ON b.member_no = m.member_no WHERE `board_type` = ? ORDER BY `hits` DESC LIMIT "+b.getLimitPage()+", "+b.getBoardList();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, boardType);
			rs = pstmt.executeQuery();
			while(rs.next()) {
	...
	public List<Board> dateASC(int boardType, Board b, Connection conn){
		...
		int count = 0;
		try {
			String sql = "SELECT * FROM `board` b JOIN `member` m ON b.member_no = m.member_no WHERE `board_type` = ? ORDER BY `board_reg_date` ASC LIMIT "+b.getLimitPage()+", "+b.getBoardList();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, boardType);
			rs = pstmt.executeQuery();
			while(rs.next()) {
